...
| Accompagnement humain (v2.4) | Satisfaction/adhésion des équipes mesurée au même instrument à T0 et à la réévaluation ⟲, participation effective aux ateliers de co-construction, relais internes formés et encore actifs après le désengagement du consultant (§Accompagnement de l'humain dans la trajectoire) |
...
> **Ce que ça ne tranche pas :** durée exacte de ③ (dépend du nombre d'équipes et du scénario, §Routage des scénarios), et qui porte la trajectoire au global — probablement `iap-operating-model-architect` en propriétaire, avec `iap-change-coach` co-portant le volet humain (cohérent avec son rôle déjà cadré en ②/③ agent), mais non tranché à ce stade. Le *contenu* de ce volet humain, lui, est désormais cadré (§Accompagnement de l'humain dans la trajectoire, v2.4 ci-dessous).

### Accompagnement de l'humain dans la trajectoire (v2.4 — transposé de l'offre SCALE)

**Le trou identifié :** la trajectoire ①②③⟲ ci-dessus séquence *ce qui* se déploie (backlog, comitologie, agents éventuels) mais réduisait l'accompagnement des *personnes* à une ligne non tranchée (« `iap-change-coach` co-portant le volet humain »). Le corpus traite l'humain là où il est périphérique — la posture d'un manager (§Focus management), l'adoption d'un agent IA (§Modèles d'équipe) — jamais l'accompagnement de ceux qui vivent le changement d'operating model lui-même : le sponsor qui doit le porter, les équipes infra qui changent de gestes, les résistances, la montée en compétence. Source de la transposition : les trois decks de l'offre OCTO SCALE (transformation agile, `docs/Import/` ; notes d'extraction sourcées page à page dans `docs/Import/notes-extraction-scale.md`). **Transposé, pas copié** : SCALE transforme une organisation entière sur 1 à 3 ans, IAP embarque un écosystème infra sur une mission de quelques semaines plus une boucle — les dispositifs changent d'échelle, les mécanismes restent.

> **Pourquoi c'est structurant, pas cosmétique :** SCALE ouvre son argumentaire sur un chiffre martelé quatre fois dans le même deck (Kotter, « Leading Change: Why Transformation Efforts Fail », HBR) : **« 70 % des transformations sont inachevées ou échouent parce que les facteurs humains et culturels sont mal pris en compte »** — et nomme cinq dimensions du changement à accompagner : méthodologique, technologique, **culturelle, sociale et émotionnelle**. La chaîne IAP couvre les deux premières ; les trois autres n'avaient aucun porteur dans la trajectoire. Un target operating model informé, priorisé et instrumenté échoue au déploiement si personne n'accompagne ceux qui doivent changer leur façon de travailler — le même angle mort que la règle d'or « jamais l'IA sur un problème d'abord organisationnel » (§Règles d'or), appliqué cette fois à la mission elle-même. L'image SCALE de l'iceberg « double hélice » nomme bien la limite actuelle du module : tout ce que produit la chaîne (diagnostic, scoring, comitologie, KPIs) travaille le **système de production** visible ; le **système social** immergé (relations, postures, structure de pouvoirs) restait sans porteur.
...
5. **Le consultant se rend progressivement dispensable** — posture explicite du coach SCALE : Former (le pourquoi) → Mentorer (montrer le geste) → Coacher (feedback, puis s'écarter), avec montée de **relais internes** (champions, communauté de coachs/référents internes — facteur de succès récurrent des références clients). C'est le mécanisme Coach → Délégué déjà cadré (§Traitement des gaspillages), appliqué au consultant lui-même.

**Le fil humain, temps par temps** — un fil dans ①②③⟲, pas un stream séparé (même logique d'intégration que la piste agents IA, §Trajectoire ci-dessus) :
...
> **Déontologie du consultant — transposée telle quelle (SCALE, « ce que le coach ne fait pas ») :** évaluer les personnes individuellement (déjà la règle des indicateurs de posture, §Focus management : « jamais un score RH déguisé ») ; faire le reporting à la place de l'équipe ; se substituer durablement à elle ; être l'intermédiaire permanent entre l'équipe et le sponsor quand ils gagneraient à se parler directement. Le critère de sortie est le miroir humain du signal `deskilling-risk` déjà cadré pour les agents (§Modèles d'équipe) : *l'équipe et ses relais tiennent-ils le modèle une période sans le consultant ?*

> **Condition d'engagement, pas une option :** le point ouvert existant « `iap-change-coach` doit-il refuser une mission sans engagement RH minimal ? » (§Points ouverts) reçoit ici son argument de fond — SCALE pose que la RH est « un frein ou le principal accélérateur » et vérifie le mandat et la disponibilité des interlocuteurs *avant* de s'engager. A minima, l'engagement personnel du sponsor et la disponibilité réelle des équipes se testent au niveau de l'intake, avant signature — au même titre que le gate IA n'a jamais été conditionné à un MVP. Le refus de mission reste à trancher (owner inchangé), mais le *test* d'engagement, lui, est acté.
